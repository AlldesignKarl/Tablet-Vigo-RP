...
export default function ShopProductsAdminPanel({ products }: { products: Product[] }) {
  const router = useRouter();
  const { push } = useToast();
  const supabase = createClient();
  const [form, setForm] = useState({ code: '', name: '', description: '', icon: '🛒', priceEuros: 0 });

  async function updateField(p: Product, patch: Partial<Product>) {
    const { error } = await supabase.from('shop_products').update(patch).eq('id', p.id);
    if (error) {
  ...
  async function addProduct(e: React.FormEvent) {
    ...
    if (error) {
      push({ kind: 'error', title: 'No se pudo crear el producto', message: error.message });
      return;
    }
    push({ kind: 'success', title: 'Producto creado' });
    setForm({ code: '', name: '', description: '', icon: '🛒', priceEuros: 0 });
    router.refresh();
  }

  return (
    <div className="space-y-6">
      <h2 className="text-lg font-bold text-white">Productos de la tienda</h2>
      <div className="space-y-3">
        {products.map((p) => (
          <div key={p.id} className="hud-panel flex flex-wrap items-center gap-3 p-4">
            ...
            <input
              ...
              onBlur={(e) => updateField(p, { price_cents: Math.round(Number(e.target.value) * 100) })}
            ...
            <label className="flex items-center gap-1.5 text-xs text-slate-400">
              <input type="checkbox" defaultChecked={p.active} onChange={(e) => updateField(p, { active: e.target.checked })} />
              Activo
            </label>
          </div>
        ))}
        {products.length === 0 && <p className="text-sm text-slate-500">Sin productos adicionales todavía.</p>}
      </div>

      <form onSubmit={addProduct} className="hud-panel grid grid-cols-1 gap-3 p-4 sm:grid-cols-3">
